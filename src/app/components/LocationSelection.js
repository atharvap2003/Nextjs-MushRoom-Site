  ...
  return (
    <Menu as="div" className="w-full h-full flex xl:flex:row">
      <div class="relative flex-1">
        ...
        {/* items  */}
        <Menu.Items className="dropdown-menu shadow-lg absolute -top-56 xl:top-[90px] left-1/2 xl:left-0 z-10 transform -translate-x-0 text-sm text-center xl:text-left w-full bg-white max-w-[332px] py-6 rounded-[10px]">
          {locations.map((location, index) => {
            return (
              <div
                onclick = {()=> setLocation(location)}
                key={index}
                className="cursor-pointer py-4 xl-pl-10 hover:bg-gray-50 text-[13px] uppercase text-center"
              >
                {location}
              </div>
            );
